...
| **Venue points** | `data/festival-pois.json` lists each point with normalized coordinates `(u, v)` on the festival map layout (0–1). At runtime they are projected onto the LVMS infield rectangle `INFIELD_BOUNDS` defined in `js/app.js`, so each pin lands on the real basemap. There is **no festival JPG overlay**; the user sees the offline OSM/CARTO basemap with pins on top. |
...
| **Offline cache** | `sw.js` precaches the app shell plus **every tile URL** from the manifest. The cache name is bumped when assets change (currently `edc-vegas-2026-v9`). |
...
GPS alignment and POI positions are **approximate**. Venue `(u, v)` values were derived from the festival map layout, not survey-grade coordinates. Adjust `INFIELD_BOUNDS` in `js/app.js` (and individual POIs in `data/festival-pois.json`) after a field check if you need tighter accuracy. Basemap © [OpenStreetMap](https://www.openstreetmap.org/copyright) contributors / © [CARTO](https://carto.com/attributions).
