...
| **Offline cache** | `sw.js` precaches the shell, map JPG, and tile list; URLs are resolved from `registration.scope` so the same files work at the site root or under a path (e.g. `/edc/`). Cache name (currently `edc-vegas-2026-v15`). |
