...
| **Offline cache** | `sw.js` precaches the shell, map JPG, and tile list; URLs are resolved from `registration.scope` so the same files work at the site root or under a path (e.g. `/edc/`). Cache name (currently `edc-vegas-2026-v14`). |
...
This repository is only the static PWA. Copy **`next.config.portfolio-rewrites.cjs`** into your portfolio (or merge its `redirects` + `rewrites` into your existing config).

That file **redirects `/edc` → `/edc/`** (required): with a `/edc` URL (no slash), the browser treats the path like a file, so `./css/app.css` resolves to `/css/app.css` and assets 404. After the redirect, `./…` resolves under `/edc/…` correctly.

Rewrites then proxy **`/edc/`** to the EDC `index.html` and **`/edc/:path*`** to the same paths on your Vercel EDC deployment.

**Basemap tiles:** the tile URL must keep literal `{z}/{x}/{y}` for Leaflet. This app builds that URL by string concat (not `new URL()`), which would encode `{` and hide the map.
