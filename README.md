...
- **Online basemap tied to theme** — When **online**, **light** mode uses standard **OpenStreetMap** raster tiles; **dark** mode uses **CARTO dark** tiles. Bundled **LVMS** raster tiles remain the **underlay** over the infield when online, and are the sole basemap when **offline** (no external tile fetches at runtime). The theme toggle remains available **offline** to switch UI chrome even though the map cannot load OSM/CARTO tiles without a network.
...
| **Basemap** | Bundled **CARTO**-style `dark_all` tiles under `tiles/{z}/{x}/{y}.png`, listed in `data/tiles-manifest.json` using **relative** paths. Generated by `scripts/fetch_osm_tiles.py`. Tiles load only from your origin (no OSM.org tile servers at runtime for the LVMS stack). When **online**, the app may also show **OpenStreetMap** standard tiles (light theme) or **CARTO dark** (dark theme) as a world layer above that underlay; see `js/app.js`. |
| **Venue points** | `data/festival-pois.json` lists each point with normalized coordinates `(u, v)` taken from the official 2026 festival map (`assets/edc_map.jpg`). The artwork is **not** north-up — Insomniac rotates the venue so it fits a portrait poster — so `js/app.js` first applies an artwork-rotation transform (`ARTWORK_ROTATION_DEG`, default **90° CCW**: top of the artwork = East at LVMS) before projecting onto `INFIELD_BOUNDS`. Translucent **stage and district fills** are separate polygon rings in **`STAGE_UV_ZONES`** inside `js/app.js` (same `(u, v)` frame as the POIs). There is **no festival JPG overlay**; the user sees the OSM/CARTO basemap (when online) or bundled tiles with pins and fills on top. |
...
- **Light and dark theme** — Header **switch** applies to the **whole app** and is saved in **`localStorage`** (`edc2026_site_theme`). When **online**, light mode uses **OpenStreetMap** tiles and dark mode uses **CARTO dark**; bundled LVMS tiles stay as the infield underlay. When **offline**, only bundled tiles load, but the UI theme still updates.
- **Live GPS** — Your position updates from `navigator.geolocation` (works without cellular data when the OS has a GPS lock).
- **Basemap and speedway** — Pan and zoom on real cartography when online (**OpenStreetMap** in light theme, **CARTO dark** in dark theme); **stage and district shading** (light translucent **non-overlapping** polygons traced in festival-map space, plus **high-contrast** area labels) sits under POIs; venue pins use the same artwork projection as those fills.
