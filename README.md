...
| **Basemap** | Bundled **CARTO** `dark_all` raster tiles (same Mercator grid as OSM) under `tiles/{z}/{x}/{y}.png`, listed in `data/tiles-manifest.json`. Generated by `scripts/fetch_osm_tiles.py` (one-time download from `basemaps.cartocdn.com` for your bbox only). The live app loads **only** `/tiles/...` from your origin so browsers never hit OSM.org tile servers (which block many deployed sites with 403). |
...
| **Offline cache** | `sw.js` precaches the app shell, the official festival map JPG, and **every tile URL** from the manifest. The cache name is bumped when assets change (currently `edc-vegas-2026-v12`). |
